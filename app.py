...
def initialize_agent(db_path: str):
    """Initialize the SQL agent with uploaded database"""
    
    db = SQLDatabase.from_uri(f"sqlite:///{db_path}")
    
    model = ChatGroq(model="qwen/qwen3-32b",temperature=0,max_tokens=None,reasoning_format="parsed",timeout=None,max_retries=2,)
    ...
    def generate_query(state: MessagesState):
        system_message = {
            ...
            Limit results to 5 unless specified otherwise. Only query relevant columns."""
        }
...
with st.sidebar:
    ...
    if st.session_state.db:
        ...
        with st.expander("Sample Data"):
            ...
            if selected_table:
                try:
                    sample_query = f"SELECT * FROM {selected_table} LIMIT 3"
                    result = st.session_state.db.run(sample_query)
...
if not st.session_state.db_path:
    ...
    3. **Get answers** - the AI will generate and run SQL queries automatically
